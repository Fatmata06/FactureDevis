<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            Modifier le Client Final
        </h2>
    </x-slot>

    <div class="py-6 max-w-3xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6">
            <form action="{{ route('clients_finaux.update', $clientFinal) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nom <span class="text-red-500">*</span></label>
                    <input type="text" name="nom" value="{{ old('nom', $clientFinal->nom) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="prenom" value="{{ old('prenom', $clientFinal->prenom) }}"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring focus:ring-indigo-200">
                    @error('prenom')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="entreprise_client_id" required
                        ...
                        @foreach($entreprises as $entreprise)
                            <option value="{{ $entreprise->id }}" {{ old('entreprise_client_id', $clientFinal->entreprise_client_id) == $entreprise->id ? 'selected' : '' }}>
                                {{ $entreprise->nom }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex justify-end space-x-2">
                    ...
                    <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">Mettre à jour</button>
                </div>
            </form>
        </div>
    </div>
